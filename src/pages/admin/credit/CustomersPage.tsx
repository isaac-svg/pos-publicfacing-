        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y">
            {isLoading ? <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-400">Loading…</td></tr> :
              list.length === 0 ? <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-400">No customers</td></tr> :
              list.map(c => (
                <tr key={c.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => window.location.href = `/credit/customers/${c.id}`}>
                  ...
                  <td className="px-4 py-2 text-center"><span className={`px-2 py-0.5 rounded text-xs ${c.riskScore === 'high' ? 'bg-red-100 text-red-700' : c.riskScore === 'medium' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}>{c.riskScore}</span></td>
                </tr>
              ))}
          </tbody>
        </table>
